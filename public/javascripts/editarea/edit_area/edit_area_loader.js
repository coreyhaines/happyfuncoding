...
EditAreaLoader.prototype ={
	...
	//	alert(settings["plugins"].length+": "+ settings["plugins"].join(","));
		...
		// ZBS:
		editAreas[s["id"]].key_up_callback = s.key_up_callback;
	},
	...
	start : function(id){
		...
		if(!d.getElementById("EditAreaArroundInfos_"+id) && (editAreas[id]["settings"]["debug"] || editAreas[id]["settings"]["allow_toggle"]))
		{
			...
			if(editAreas[id]["settings"]["debug"])
				html+="<textarea id='edit_area_debug_"+ id +"' spellcheck='off' style='white-space:pre; z-index: 20; width: 100%; height: 120px;overflow: auto; border: solid black 1px;'></textarea><br />";				
